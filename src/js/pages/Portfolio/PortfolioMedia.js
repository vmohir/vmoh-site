...
export default class PortfolioMedia extends React.Component {
  render() {
    ...
    return (
      <li className={'vertical-slider-item portfolio-media-item ' + mainListItem_noImgClass}>
        {imgComp}
        <a className="portfolio-media-view-link" href={link} target="_blank">
          View link
        </a>
        <div className="portfolio-media-content">
          <h1>{title}</h1>
          <p>{desc}</p>
          <ul className="portfolio-media-techs">{technologiesComp}</ul>
        </div>
      </li>
    );
  }

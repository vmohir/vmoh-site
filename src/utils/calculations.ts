...
export function calculatePersonTotals(
  ...
  items.forEach(item => {
    ...
    item.paidBy.forEach((payer, personId) => {
      const data = personData.get(personId);
      if (data) {
        const paidInBaseCurrency = convertCurrency(payer.amount, payer.currency, baseCurrency);
  ...
  const billSubtotal = items.reduce((sum, item) => {
    const priceInBaseCurrency = convertCurrency(item.price, item.currency, baseCurrency);
